...
raw_data <- read_csv("/Users/macbookpro/Desktop/AAcXZqa6.csv")
...
gss <- raw_data %>% 
  ...
  mutate_at(vars(smg_01:agegr10), .funs = funs(ifelse(.>=96, NA, .))) %>% 
  mutate_at(.vars = vars(smg_01:agegr10),
...
gss <- gss %>% 
  clean_names() %>% 
  rename(level_of_stress = smg_01,
         smoking = smk_05,
         sex = sex,
         province = prv,
         number_of_children = chinhsdc,
         education_level = ede_01,
         agegroup = agegr10
  ) 
